...
(function () {
    ...
    if (contactForm) {
        ...
        const baseMsg = messages[lang] || messages.en;
        const msg = {
            sending: contactForm.dataset.msgSending || baseMsg.sending,
            success: contactForm.dataset.msgSuccess || baseMsg.success,
            error: contactForm.dataset.msgError || baseMsg.error,
            required: contactForm.dataset.msgRequired || baseMsg.required,
            invalidEmail: contactForm.dataset.msgInvalidEmail || baseMsg.invalidEmail
        };
        ...
        contactForm.addEventListener('submit', async function (e) {
            ...
            if (submitButton) {
                ...
                if (submitSpan) {
                    submitSpan.textContent = msg.sending;
                } else {
            ...
            try {
                ...
                if (submitButton) {
                    ...
                    const submitSpan = submitButton.querySelector('span');
                    const originalSpanText = lang === 'ca' ? 'Enviar Missatge' : 'Send Message'; // Fallback logic

                    // Actually, the originalButtonText was captured at line 96.
                    // Let's use it properly.
                    if (submitSpan) {
                        // Extract just the text parts from the original button text if needed, 
                        // but better to just restore the original text if we targeted the span.
                        submitSpan.textContent = originalButtonText.trim();
                    } else {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Individual scroll-reveal elements
  document.querySelectorAll('.scroll-reveal').forEach(el => observer.observe(el));

  // Stagger grid containers + section headers
  document.querySelectorAll(
    '.app-features-grid, .benefits-grid, .pricing-grid, .metrics-grid, .section-header'
